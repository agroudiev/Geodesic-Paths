...
\begin{frame}{Introduction}
	\begin{figure}
		...
		\caption{Geodesic paths from a source point on a bunny, a bust and a icosahedron.}
	\end{figure}
\end{frame}
%
%
...
\begin{frame}{Compute time: Heat Method}
	\begin{figure}
...
\begin{frame}{Compute time: Fast Marching}
	\begin{figure}
...
\begin{frame}{Comparison of methods}
	\begin{tblr}[expand=\intertitre]{%
		...
		colspec={Q[co=1,c, font=\small]Q[co=1.5,mode=math,c,font=\footnotesize]Q[mode=math,co=1.5,c,font=\footnotesize]Q[co=1.2,c, font=\small]Q[c, co=1.5, font=\footnotesize]},
		hlines = {solid},
		vline{1, 2, 6} = {solid},
		vline{3, 4, 5} = {1}{solid},
		vline{3, 4, 5} = {2-Z}{dashed},
		row{3, 5, 8}={bg=main-below!20},
				row{1} = {c,font=\bfseries\small,mode=text,fg=main-accents,bg=main-below},
				colsep = 3pt
			}%
		Method        & Theoretical Runtime & Actual Runtime   & Precision & \small Main limits \\
		Heat          & \O(\nv^{\omega})    & \O(\nv^{\omega}) & Approx.   & No paths           \\
		Poisson       & \O(\nv^{\omega})    &                  & Approx.   & Not geodesics      \\
		Spectral      & \O(\nv^{\omega})    &                  & Approx.   & Not geodesics      \\
		Fast Marching & \O(\nv\log \nv)     & \O(\nv^{1.5})    & Approx.   & Fixed sources      \\
		ICH           & \O(\nv^{2}\log \nv) & \O(\nv^{1.6})    & Exact     & Limited to meshes
	\end{tblr}
\end{frame}
