...
In this section we will study methods inspired by physical equations.
...
Finally, we can define the Laplace-Beltrami operator (the piecewise-linear version of the continuous laplacian) as $\Delta = \div \cdot \nabla : \V \to \R$ or simply
\begin{equation*}
...
In our implementation, we compared multiple methods allowing to trace geodesics.
Not all of those will actually compute the true geodesic distance, but something that can be drawn to it.

\paragraph{Heat Method}
This method is based on the heat equation $\nabla u_{t} = \frac{\d}{\d t}u_{0}$.
We discretize it as:
\begin{equation*}
...
then solving the Poisson equation $\Delta \phi = \div X$ to retrieve the true distance function.
...
The equations in this paragraph all allow to draw geodesics, although they do not give
