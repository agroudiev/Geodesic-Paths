...
\title[Geodesic Paths and Distances on Meshes]{Geodesic Paths and Distances on Meshes: PDE and Computational Geometry Approaches\texorpdfstring{\\{\normalsize Report on \textit{A Survey of Algorithms for Geodesic Paths and Distances}}}{}}
...
When considering a $2$-manifold $M$, a (smooth) curve is a (smooth) application $\gamma: I \to M$.
...
The following equations all implicate the Laplace-Beltrami operator (or laplacian operator $\Delta$) which
...
Adding this to the symmetry at the core of the Laplacian's definition (in a local coordinate frame), it is
bound to appear when moreover the laws of deviation are symmetric, which is often the case in physics.
...
being covariant\footnote{Considering the linear category of differential operators between tensor fields,
	we can prove that the laplacian is generated by the covariant derivative\footnote{Which in Riemannian
		geometry is just the derivative along a vector adjusted to respect the Riemannian tensor metric of the
		manifold.}.}, and is fundamental as such.
...
The Poisson equations we consider are the screened Poisson equation
\begin{equation*}
	\begin{cases}
	...
	\end{cases}
\end{equation*}
and the border Poisson equation
\begin{equation*}
	\begin{cases}
...
sense of distance on the manifold, which is however not the geodesic distance on the embedding in $\R^{3}$.
...
starting at $x$.
For the diffusion distance, the discrete diffusion process used for definition is simply considering the
amount of information that has been diffused from $x$ to $y$ and from $y$ to $x$ at time $t$ and is
directly tied to the heat equation as such.

\smallskip

As said, these methods do not contain the geodesics, but may be used as good approximations of the geodesic
distance (or for their proper interpretation) as regularized versions of the geodesic distance.
Moreover, when considering $k$ much smaller than $\nv$, it is easy to approximate the distance quite well
(see Figure \ref{fig:spectral-approx} below).
...
Those methods are thus much less adapted to the problem of computing geodesics, although they solve a similar class of problems.
...
In this section, we present experiments and results obtained from our implementations of the heat method, fast marching, and the Improved Chen-Han algorithm.
All methods being implemented from scratch in the same programming language (Rust), and optimized up to a similar level, we believe the comparison to be fairly representative of the practical performances of each algorithm.
...
\textcolor{red}{TODO: explain theoretical runtime}
In Figure \ref{fig:heat-experiments-results}, we present the results of our benchmark of the practical runtime of the heat method on various meshes. We do not observe a clear linear trend in log-log scale as a function of the number of vertices on the selected range of sizes. We believe this is due to the overhead of \textcolor{red}{TODO} for small meshes, and that the asymptotic behavior would be more visible on larger meshes.
...
In Figure \ref{fig:spectral-approx} we present how well the approximation
\begin{figure}[ht]
	SCHPOUET
