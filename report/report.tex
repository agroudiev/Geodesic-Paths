...
\begin{figure}[ht]
	...
	\end{tikzpicture}
	\caption{Example of suboptimality of applying Dijkstra's algorithm on the graph formed by the vertices and edges of the mesh. The \textcolor{purple}{red} path follows the edges of the mesh, while the \textcolor{teal}{blue} path is the true geodesic path on the 2D surface, which is shorter.}
	\Description{}
...
\begin{figure}[ht]
	...
	\end{tikzpicture}
	\caption{Illustration of the ``one angle, one split'' pruning rule: if two windows on the same edge ${\color{blue}e}$ split on the same edge $v$, only three out of the four resulting windows are kept.}
	\Description{}
...
\begin{figure}[ht]
	...
	\end{tikzpicture}
	\caption{Illustration of the window filtering in ICH. If one the following inequalities holds, the window $w$ can be safely discarded:}
	\Description{}
