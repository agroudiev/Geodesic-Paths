...
\begin{figure}[t]
	\centering
	\includegraphics[width=.35\linewidth]{images/manifold.png}
	\includegraphics[width=.45\linewidth]{images/nonmanifold.png}
...
the same edge to avoid redundancy later on; this simple trimming strategy is illustrated in
\autoref{fig:window-trimming-mmp}.
...
For each mesh, each algorithm has been run 10 times with different source vertices, each point
corresponding to the average runtime over these runs.
...
specific geometry of the meshes used in the benchmark, and may not generalize to all possible
meshes.
