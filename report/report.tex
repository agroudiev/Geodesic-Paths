...
The \imp{edges} $\E$ of the manifold are given by any subsets of size two of a face $f \in \F$.
...
properties\footnote{For example, knowing the electromagnetic light field on a plane allows to derive it
	uniquely on all of space.}.
...
Consider for this example the parabolic heat equation
\begin{equation*}
...
However, on a PL manifold, because functions on vertices are interpolated linearly to
...
being perpendicular to level curves is
\begin{equation}
...
being covariant\footnote{Considering the linear category of differential operators between tensor fields,
	one can prove that the Laplacian is generated by the covariant derivative ; which in Riemannian
...
transform well enough (see Figure \autoref{fig:spectral-approx} below).
...
\begin{table}[t]
	\begin{tblr}[expand=\intertitre]{%
		...
		colspec={Q[co=1,c]Q[co=1.5,mode=math,c]Q[mode=math,co=1.5,c]Q[co=1.2,c]Q[c, co=1.5, font=\scriptsize]},
		hlines = {solid},
		vline{1, 2, 6} = {solid},
		vline{3, 4, 5} = {1}{solid},
