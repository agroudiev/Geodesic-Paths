...
In theory, the method computes the laplacian in $\O(\nv^{2})$ then solves two linear systems in
about $\O(n^{\omega})$ the matrix multiplication complexity (in practice, we should have $\omega \leq \log_{2}(7) = 2.83$ but not much smaller).
However, we do not observe a clear linear trend in log-log scale as a function of the number of vertices
...
small meshes, which takes a lot of added operations, as well as the overhead in the complexity constant
hidden in the big O notation that the most asymptotically efficient algorithms for matrix multiplication
and system solving induce and that the asymptotic behavior would be more visible on larger meshes.
...
\begin{figure}[ht]
	...
	\caption{$\ell^{2}$ error of the spectral embedding on $k$ vectors to the full spectral embedding.
		...
		$\nv$ used for the embedding size. \textcolor{red}{TODO: \@ antoine ça te convient}}
	\Description{}
...
optimal in distance, we cannot reconstruct paths from those methods.
