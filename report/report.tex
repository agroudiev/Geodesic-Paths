...
Such a problem arises in various fields, including computer graphics, robotics, medical imaging, and computer vision. In computer graphics, geodesic distances are used for texture mapping; in robotics, they form the cornerstone of path planning algorithms on complex terrains; in medical imaging, they assist in analyzing anatomical structures; and in computer vision, they improve object recognition and shape analysis.
...
\todo{Reformuler la phrase ci-dessous}
Because we cannot have continuous functions, functions on the manifold will be represented as functions
on $\V$, on $\E$ or on $\F$.
...
\begin{figure}[ht]
	...
	\caption{Example of suboptimality of applying Dijkstra's algorithm on the graph formed by the vertices and edges of the mesh. The red path follows the edges of the mesh, while the blue path is the true geodesic path on the surface, which is shorter.}
	\Description{}
...
\subsection{ICH practical runtime}
...
Each mesh was processed by our implementation of the ICH algorithm, starting from a single source vertex chosen arbitrarily.
For each mesh, the algorithm is run \textcolor{red}{TODO: number} times with different source vertices, the points in \autoref{fig:ich-experiments-results} corresponding to the average runtime over these runs.

\begin{figure}[ht]
	\centering
	\includegraphics[width=.9\linewidth]{images/ich_benchmark.pdf}
	\caption{\textcolor{red}{TODO: increase figure size} Benchmark of the time complexity of the ICH algorithm on various 3D meshes, as a function of the number of vertices. Slope: $1.58$, $r^2$: $0.98$.}
	\Description{}
	\label{fig:ich-experiments-results}
\end{figure}

The results confirm the sub-quadratic practical runtime of the ICH algorithm, with a fitted slope of $1.58$ on a log-log scale, corresponding to a time complexity of approximately $\mathcal{O}(n^{1.58})$ in practice. Care should be taken when interpreting this result, as the practical runtime may depend on the specific geometry of the meshes used in the benchmark, and may not generalize to all possible meshes. The number of vertices is not always a fair indicator of the complexity of the geometry, and other factors such as the distribution of vertex valences and curvature may also impact the practical runtime of the algorithm.
