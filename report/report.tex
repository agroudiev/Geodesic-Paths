...
Such an homeomorphism is called a chart.
...
\begin{equation*}
	\begin{array}{rccl}
		(eigenmap)     & \phi_{E}^{2}(x, y) & = & \sum_{i = 1}^{k} \abs{\psi_{i}(x) - \psi_{i}(y)}^{2}                          \\
		(commute time) & \phi_{C}^{2}(x, y) & = & \sum_{i = 1}^{k} \frac{1}{\lambda_{i}}\abs{\psi_{i}(x) - \psi_{i}(y)}^{2}     \\
		(biharmonic)   & \phi_{B}^{2}(x, y) & = & \sum_{i = 1}^{k} \frac{1}{\lambda_{i}^{2}}\abs{\psi_{i}(x) - \psi_{i}(y)}^{2} \\
		(diffusion)    & \phi_{D}^{2}(x, y) & = & \sum_{i = 1}^{k} e^{-2t\lambda_{k}} \abs{\psi_{i}(x) - \psi_{i}(y)}^{2}       \\
	\end{array}
\end{equation*}
...
Some of the issues that arise with those methods have already been described above, because they are all
based on the quite slow solving of linear systems, but the main issue with those methods is that they only
allow to describe some distance transform from sources/some distance transform on the step.
Those methods are thus much less adapted to the problem of computing geodesics, although they solve a class
of similar problems.
...
For each mesh, each algorithm is run \textcolor{red}{TODO: number} times with different source vertices, each point corresponding to the average runtime over these runs.
