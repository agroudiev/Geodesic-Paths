...
\begin{itemize}
	...
	\item If needed, we discretize in time the equation with a single step of backwards Euler method as done in Equation \eqref{eq:discrheat}.
...
We then sort those by increasing eigenvalues, skip the first one and, for different values of $k$ the
embedding size, compute the approximate geodesic.
